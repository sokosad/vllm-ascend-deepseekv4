...
class MoEWeights:
    ...
    # Pool (redundant) weights — separate tensors so the pool can have a
    # different expert count per layer (craft_alloc MCKP), decoupled from the
    # main tensor's ep-aligned size. None when pool is disabled (default).
