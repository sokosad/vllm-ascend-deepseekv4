...
class AscendFusedMoE(FusedMoE):
    ...
    def __init__(self, *args, **kwargs):
        ...
        global_pool_total_size = get_configured_craft_global_pool_size(eplb_config)
        global_pool_size = get_craft_global_pool_size_per_rank(eplb_config, self.ep_size)
        self.craft_global_pool_enabled = global_pool_total_size > 0
        quant_scheme = getattr(self.quant_method, "quant_method", None)
        if self.craft_global_pool_enabled and not getattr(
        ...
        configured_pool_mode = (
            ...
            or self.craft_global_pool_enabled
        )
        configured_pool_size = (
            global_pool_size
            if self.craft_global_pool_enabled
            else get_configured_craft_pool_size(eplb_config, self.moe_instance_id)
        )
        ...
        if envs_ascend.VLLM_ASCEND_LOG_CARD_TOKENS and self.global_expert_map is not None:
            logger.info_once("[PLACEMENT] layer=%d ep_rank=%d expert_map=%s",
                             self.moe_instance_id, self.ep_rank, self.global_expert_map.tolist())
        ...
        if eplb_config.dynamic_eplb and eplb_config.eplb_policy_type == 4 and not self.craft_pool_enabled:
            raise ValueError(
                ...
                "or craft_global_pool_size > 0 or a pool_mode expert_map_path."
            )
        ...
        if self.dynamic_eplb or envs_ascend.VLLM_ASCEND_LOG_CARD_TOKENS:
...
# ===== Co-placement profiling (env-gated, default off) =====
_COOC_STATE: dict = {}
_CT_STATE: dict = {}
_DUMP_STATE: dict = {}
_DUMP_DIR_CACHE: str = ""
...
def _log_logical_topk_cooccurrence(topk_ids, layer_id, ep_rank):
    """Pair-level expert coactivation. Gated by VLLM_ASCEND_LOGICAL_TOPK_COOC_LOG."""
    try:
        from vllm_ascend import envs as _env
        if not _env.VLLM_ASCEND_LOGICAL_TOPK_COOC_LOG:
            return
    except Exception:
        return
    try:
    ...
    if topk_ids.dim() != 2 or topk_ids.numel() == 0:
        return
    key = (int(layer_id), int(ep_rank))
    cc = _COOC_STATE.get(key, 0) + 1
    _COOC_STATE[key] = cc
    try:
        interval = max(1, int(_env.VLLM_ASCEND_LOGICAL_TOPK_COOC_INTERVAL))
    except Exception:
        interval = 200
    if cc % interval != 0:
        return
    ids_cpu = topk_ids.detach().to("cpu", non_blocking=False).to(torch.int64)
    expert_counts = {}
    pair_counts = {}
    for _row in ids_cpu.tolist():
        _ex = sorted({int(_e) for _e in _row if int(_e) >= 0})
        for _e in _ex:
            expert_counts[_e] = expert_counts.get(_e, 0) + 1
        for _i, _l in enumerate(_ex):
            for _r in _ex[_i + 1:]:
                pair_counts[(_l, _r)] = pair_counts.get((_l, _r), 0) + 1
    try:
        _tp = max(1, int(_env.VLLM_ASCEND_LOGICAL_TOPK_COOC_TOP_PAIRS))
    except Exception:
        _tp = 8
    from vllm.logger import logger as _lg
    _lg.info("[Logical TopK Cooccur] layer=%d ep_rank=%d samples=%d tokens=%d topk=%d top_experts=%s top_pairs=%s",
             int(layer_id), int(ep_rank), cc, int(ids_cpu.shape[0]), int(ids_cpu.shape[1]),
             sorted(expert_counts.items(), key=lambda x: (-x[1], x[0]))[:_tp],
             sorted(pair_counts.items(), key=lambda x: (-x[1], x[0]))[:_tp])


def _log_logical_topk_ct(topk_ids, layer_id, ep_rank, n_experts, ep_size):
    """Token-level C_T. Gated by VLLM_ASCEND_LOGICAL_TOPK_CT_LOG."""
    try:
        from vllm_ascend import envs as _env
        if not _env.VLLM_ASCEND_LOGICAL_TOPK_CT_LOG:
            return
    except Exception:
        return
    try:
        ...
        pass
    if topk_ids.dim() != 2 or topk_ids.numel() == 0 or ep_size <= 0 or n_experts <= 0:
        return
    key = (int(layer_id), int(ep_rank))
    cc = _CT_STATE.get(key, 0) + 1
    _CT_STATE[key] = cc
    try:
        interval = max(1, int(_env.VLLM_ASCEND_LOGICAL_TOPK_CT_INTERVAL))
    except Exception:
        interval = 200
    if cc % interval != 0:
        return
    ids_cpu = topk_ids.detach().to("cpu", non_blocking=False).to(torch.int64)
    experts_per_rank = max(1, n_experts // ep_size)
    ct_hist = {}
    ct_sum = 0
    ct_count = 0
    for _row in ids_cpu.tolist():
        _eids = {int(_e) for _e in _row if int(_e) >= 0}
        _ranks = {_e // experts_per_rank for _e in _eids}
        _ct = len(_ranks)
        ct_hist[_ct] = ct_hist.get(_ct, 0) + 1
        ct_sum += _ct
        ct_count += 1
    _ct_mean = ct_sum / ct_count if ct_count > 0 else 0.0
    from vllm.logger import logger as _lg
    _lg.info("[Logical TopK CT] layer=%d ep_rank=%d samples=%d ct_hist=%s ct_mean=%.4f",
             int(layer_id), int(ep_rank), cc, dict(sorted(ct_hist.items())), _ct_mean)


def _dump_topk_ids(topk_ids, layer_id, ep_rank):
    """Dump raw topk_ids to .npy for offline dry-run. Gated by VLLM_ASCEND_COLOC_DUMP."""
    global _DUMP_DIR_CACHE
    try:
        from vllm_ascend import envs as _env
        if not _env.VLLM_ASCEND_COLOC_DUMP:
            return
    except Exception:
        return
    try:
    ...
    if topk_ids.dim() != 2 or topk_ids.numel() == 0:
        return
    key = (int(layer_id), int(ep_rank))
    cc = _DUMP_STATE.get(key, 0) + 1
    _DUMP_STATE[key] = cc
    try:
        interval = max(1, int(_env.VLLM_ASCEND_COLOC_DUMP_INTERVAL))
    except Exception:
        interval = 200
    if cc % interval != 0:
        return
    import os
    dump_dir = _env.VLLM_ASCEND_COLOC_DUMP_DIR
    os.makedirs(dump_dir, exist_ok=True)
    ids_cpu = topk_ids.detach().to("cpu", non_blocking=False).to(torch.int64)
    fname = os.path.join(dump_dir, f"layer{int(layer_id)}_rank{int(ep_rank)}_sample{cc}.npy")
    import numpy as _np
    _np.save(fname, ids_cpu.numpy())


def _coloc_profile(topk_ids, layer_id, ep_rank, n_experts=256, ep_size=8):
    """Call all three profiling functions. One-liner for call sites."""
